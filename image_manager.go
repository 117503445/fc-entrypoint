...
func PrepareImage(image, username, password string) (string, error) {
	return PullImageWithDedup(image, username, password)
}

// pullImageInternal actually pulls an image and creates the rootfs tar
// This is called by PullImageWithDedup
func pullImageInternal(image, username, password string) (string, error) {
	...
	client := NewRegistryClient(username, password)
	...
	if _, err := os.Stat(rootfsPath); err == nil {
		log.Info().Str("path", rootfsPath).Msg("Rootfs already exists, skipping layer download and merge")
