...
func ExecuteInChroot(ctx context.Context, rootfsTar, processDir, command, workingDir string, stdout, stderr io.Writer) error {
	// Extract rootfs to process directory
	if err := extractTar(ctx, rootfsTar, processDir); err != nil {
		return fmt.Errorf("failed to extract rootfs: %w", err)
	}

	log.Ctx(ctx).Info().Str("rootfs", processDir).Str("command", command).Msg("Executing in chroot")
...
func extractTar(ctx context.Context, tarPath, destDir string) error {
	...
	for {
		...
		if !isPathSafe(destDir, targetPath) {
			log.Ctx(ctx).Warn().Str("path", header.Name).Msg("Skipping potentially unsafe path")
			continue
		}
		...
		case tar.TypeLink:
			...
			if err := os.Link(linkTarget, targetPath); err != nil {
				...
				log.Ctx(ctx).Warn().Str("target", targetPath).Err(err).Msg("Hard link failed, will skip")
			}

		case tar.TypeChar, tar.TypeBlock:
			// Skip device files - they require root and are typically not needed
			log.Ctx(ctx).Debug().Str("path", header.Name).Msg("Skipping device file")

		case tar.TypeFifo:
			// Skip FIFO files
			log.Ctx(ctx).Debug().Str("path", header.Name).Msg("Skipping FIFO file")
	...
	log.Ctx(ctx).Info().Str("path", destDir).Msg("Extracted rootfs")
	return nil
}
